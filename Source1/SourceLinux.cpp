...
void LoadPopsHGDP(const std::string &SF);
void LoadReference(const std::string &SF, int chrom);
...
int main() {
    ...
    std::string selectedFolder = SelectFolder();
    if (selectedFolder.back() != '/') selectedFolder += '/';  
...
void LoadReference(const std::string &SF, int chrom) // loads reference sequence and counts the numbers of each triplet (TRIPS[])
{
...
void LoadPopsHGDP(const std::string &SF)
// read in population and region codes for each of 2504 individuals
{

    std::string filePath = "/mnt/h/Human_Genomes/inpopsHGDP.txt";
    ...
    if (!inputFile) {
        ...
        perror("Error details");  // This will print more details about the failure
    }

    // Read from the file and print its contents (example)
    std::string line;
...
std::cout << infile << "\n";
    ...
    if (!in1) std::cout << infile << " is NOT OPEN", exit(0);
    else std::cout << infile << " OPEN\n"; 
...
// Function to get folder path input from the user (Linux version).
std::string SelectFolder() {
    string folderPath;
    cout << "Please enter the full path to the folder containing input files: ";
    getline(cin, folderPath);  // Prompt the user to enter the folder path
    return folderPath;
}