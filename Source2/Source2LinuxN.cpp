...
void LoadPopsHGDP(std::string &SF);
void LoadPopDetailsHGDP(std::string &SF);
void LoadReference(std::string &SF, int chrom);
                ...
                if ((test1 || test2) && j != i) {
                    ...
                    if (pops[i + 5][0] == pops[j + 5][0]) {  // sampe population, conduct spiked and unspiked analysis                       
                        ...
                        if (test2) {
                            ...
                            if (geno[0][0] == A1 && geno[0][0] != A2) BCH[9]++; //branch to cmp P1 v P2
                            if (geno[0][0] == A2 && geno[0][0] != A1) BCH[8]++;
...
void LoadReference(std::string &SF, int chrom) // loads reference sequence and counts the numbers of each triplet (TRIPS[])
{
...
void LoadPopsHGDP(std::string &SF) // read in population and region codes for each of 929 individuals
{
    int N_ind[60]{};
    int N_reg[60]{};
